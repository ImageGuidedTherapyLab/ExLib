 ...
 * metric changed to be mean squares.
 */
...
int main( int argc, char *argv[] )
{
  ...
  /* If we were running this on label values, we might want to use a
  ...
  if ((*initialTransformFile) != "") {
    ...
    if (transforms->size() == 1) {
      ...
      if (!strcmp((*trsfit)->GetNameOfClass(),"VersorRigid3DTransform"))
      {
        ...
        // transform->SetRotation( trsf_read->GetRotation() );
        transform->SetMatrix( trsf_read->GetMatrix() );
    ...
    // 2. the translation
    // 3. the matrix
    // Documentation warns to set center first, then translation, 
    //     then the matrix.
    // The reading code has also initialized the transform...
