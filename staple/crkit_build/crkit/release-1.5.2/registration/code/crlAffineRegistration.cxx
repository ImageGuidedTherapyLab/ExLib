/*
 * Copyright 2010 Children's Hospital Boston
 *
...
public:
  ...
  // Set writePyramidImagesToDisk to be true to write out the image pyramid.
   ...
   std::cout <<
    ...
    std::cout <<
     ...
     OptimizerPointer optimizer = dynamic_cast< OptimizerPointer >(
      ...
      std::cout << "Paramaters" << optimizer->GetCurrentPosition() << std::endl;
      std::cout << std::endl;
    }
};
...
int main( int argc, char *argv[] )
{
  ...
  try
    {
      ...
      TCLAP::UnlabeledValueArg<std::string> initialTransformArg("initialTransform","Optional Initial Rigid Body Transform File",false,"","optional initial transform file name",cmd);
      ...
      TCLAP::ValueArg<std::string> metricNameArg("","metricName","Name of metric",false,"","name of metric (string: one of mi, normmi, meansquares, normcorr, matchcardinality)",cmd);
  ...
  MeanSquaresMetricType::Pointer metricMeanSquares =
  ...
  } else if (metricName->compare("meansquares") == 0) {
    metricBase = static_cast<MeanSquaresMetricType *>(
                      (metricMeanSquares).GetPointer() );
    std::cout << "Metric is: meansquares" << std::endl;
  } else if (metricName->compare("matchcardinality") == 0) {
    metricBase = static_cast<MatchCardinalityMetricType*>(
                      (metricMatchCardinality).GetPointer() );
    std::cout << "Metric is: matchcardinality" << std::endl;
  } else {
    std::cerr << "Don't know about metric called " << (*metricName) <<std::endl;
    std::cerr << "Try: mi, normmi, meansquares, matchcardinality or normcorr" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!strcmp(
    ...
    metricMattesMI->ReinitializeSeed( seed );
      ...
      // The default is to measure the percentage of labels that match.
      metricMatchCardinality->MeasureMatchesOn();
      optimizer->SetMaximize( true ); // Push percentage of matches to be high.
  } else if (!strcmp(
  ...
  */
  ...
  //
  RegistrationIterationUpdate::Pointer regCommand = RegistrationIterationUpdate::New();
