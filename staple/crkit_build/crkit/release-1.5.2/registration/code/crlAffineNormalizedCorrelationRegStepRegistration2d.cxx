...
public:
  ...
  typedef   const BaseOptimizerType   *    BaseOptimizerPointer;
...
int main( int argc, char *argv[] )
{
  ...
  RegularStepGradientDescentOptimizerType::Pointer regstepOptimizer =
  ...
  BaseOptimizerType::Pointer         optimizerBase;

  optimizerBase = static_cast < RegularStepGradientDescentOptimizerType *>(
                          (regstepOptimizer).GetPointer() );
  ...
  registration->SetOptimizer(     optimizerBase     );
  ...
  if (!strcmp(
   ...
   NormalizedMutualInformationMetricType::HistogramType::SizeType histogramSize;
     ...
     //  API bug - optimizer base class doesn't have a SetMaximize().

     // Push joint entropy H(A,B) to be small,
                                     // so the metric gets larger.
     // Set for all optimizers to defeat the API bug.
     powellOptimizer->SetMaximize( true ); 
     regstepOptimizer->SetMaximize( true ); 
  } else {
  ...
  typedef BaseOptimizerType::ScalesType       OptimizerScalesType;
  ...
  optimizerBase->SetScales( optimizerScales );

  /* For PowellOptimizer: */
  if (!strcmp(
        optimizerBase->GetNameOfClass(),
             "PowellOptimizer")) {
    powellOptimizer->SetMaximumIteration( numberOptimizerIterations ) ;
    powellOptimizer->SetStepLength( PowellStepLength ) ;
    powellOptimizer->SetStepTolerance( PowellStepTolerance ) ;
  } else if (!strcmp(
        optimizerBase->GetNameOfClass(),
             "RegularStepGradientDescentOptimizer")) {
    regstepOptimizer->SetNumberOfIterations( numberOptimizerIterations ) ;
    regstepOptimizer->SetMaximumStepLength( PowellStepLength ) ;
    regstepOptimizer->SetMinimumStepLength( PowellStepLength ) ;
  } else {
    std::cout << "No optimizer parameter settings for " <<
          optimizerBase->GetNameOfClass() << std::endl;
  }

  // Create the Command observer and register it with the optimizer.
  //
  CommandIterationUpdate::Pointer observer = CommandIterationUpdate::New();
  optimizerBase->AddObserver( itk::IterationEvent(), observer );
  ...
  BaseOptimizerType::ParametersType finalParameters =
  ...
  if (!strcmp(
    ...
    bestValue = powellOptimizer->GetValue();
        ...
        optimizerBase->GetNameOfClass(),
             "RegularStepGradientDescentOptimizer")) {
    numberOfIterations = regstepOptimizer->GetCurrentIteration();
    bestValue = regstepOptimizer->GetValue();
  } else {
    std::cout << "Warning: running on unknown optimizer." <<
          optimizerBase->GetNameOfClass() << std::endl;
  }
