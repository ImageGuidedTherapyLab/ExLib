/*=========================================================================
  ...
  Module:    $RCSfile: itkOneTensorEstimateFilter.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkOneTensorEstimateFilter_txx
#define __itkOneTensorEstimateFilter_txx

#include "itkOneTensorEstimateFilter.h"
...
OneTensorEstimateFilter< TReferenceImagePixelType,
  TGradientImagePixelType, TTensorPixelType >
::OneTensorEstimateFilter()
{
...
template< class TReferenceImagePixelType, 
          class TGradientImagePixelType, class TTensorPixelType >
void OneTensorEstimateFilter< TReferenceImagePixelType,
  TGradientImagePixelType, TTensorPixelType >
                       ...
                       int ) 
{
  ...
  if( m_GradientImageTypeEnumeration == GradientIsInManyImages )
    ...
    while( !it.IsAtEnd() )
      ...
      ReferencePixelType b0 = it.Value();
  ...
  // The gradients are specified in a single multi-component image
    ...
    for(GradientDirectionContainerType::ConstIterator gdcit = this->m_GradientDirectionContainer->Begin();
      ...
      {
		  double nn=gdcit.Value().two_norm();
      if(gdcit.Value().one_norm() <= 0.0)
        {
        baselineind.push_back(gdcit.Index());
        }
      else if ( nn>0.9 && nn<1.1 )		// ADD ONLY 1-norm gradients!
        {
...
::ComputeTensorBasis()
{
  ...
  for(GradientDirectionContainerType::ConstIterator gdcit = this->m_GradientDirectionContainer->Begin();
    ...
    {
		double nn=gdcit.Value().two_norm();
    if(gdcit.Value().one_norm() > 0.0 && (nn>0.9) && (nn<1.1))
      {
      gradientind.push_back(gdcit.Index());
      }
    }
                        ...
                        const GradientImagesType *gradientImage )
{
  ...
  this->m_NumberOfGradientDirections = 0;
  ...
  for(GradientDirectionContainerType::Iterator it = this->m_GradientDirectionContainer->Begin();
    ...
    if(it.Value().one_norm() <= 0.0)
      ...
      {
		double nn = it.Value().two_norm();
		
		if ( nn>0.9 && nn<1.1) this->m_NumberOfGradientDirections++;
		// DON T NORMALIZE
