...
#include "crlImageAddMultiplyAdd.h"
...
#define INSTANTIATECLASS(dim, scalar) do { \
           if (imageIODimensionality == dim) { \
             crlImageAddMultiplyAddPtr = \
       crlImageAddMultiplyAdd< dim, scalar >::New(); \
           } \
         } while (0)
...
// Calculation should involve a floating point type,
...
#define INSTANTIATEVECTORCLASS(spatialdim, componenttype, vectordim) do { \
           if (imageIODimensionality == spatialdim) { \
             ...
             crlImageAddMultiplyAddPtr = \
       crlImageAddMultiplyAdd< spatialdim, \
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 6) {
    std::cout << 
      "Usage: " << argv[0] 
                << " inmage add1 multiply add2 outimage" << std::endl;
    std::cout << "Calculates voxelwise outimage = (inimage + add1)*mult1 + add2" << std::endl;
    exit(1);
  }

  crlImageAddMultiplyAddBase::Pointer crlImageAddMultiplyAddPtr = 0;
  ...
  crlImageAddMultiplyAddPtr->SetInputImageFileName( argv[1] );
  crlImageAddMultiplyAddPtr->SetAdd1( argv[2] );
  crlImageAddMultiplyAddPtr->SetMultiply( argv[3] );
  crlImageAddMultiplyAddPtr->SetAdd2( argv[4] );
  crlImageAddMultiplyAddPtr->SetOutputImageFileName( argv[5] );

  int ret = crlImageAddMultiplyAddPtr->Execute();
  if (ret != 0) {
    std::cerr << argv[0] << " returned an error code " << ret <<
    std::endl;
    return EXIT_FAILURE;
  }
