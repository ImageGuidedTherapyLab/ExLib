...
int main(int argc, char *argv[])
{
  ...
  if (argc != 9) {
    std::cout << 
      "Usage: " << argv[0] << " labelimage innerlabel outerlabel"
       << " solveregionlabel outputLaplaceImage outputTangentField" 
       << " thicknessImageFileName displacementFieldFileName " 
       << std::endl;
    return EXIT_FAILURE;
  }
  ...
  crlImageLaplacePtr->SetOutputFileName( argv[5] );
  crlImageLaplacePtr->SetOutputTangentFieldFileName( argv[6] );
  crlImageLaplacePtr->SetOutputThicknessFileName( argv[7] );
  crlImageLaplacePtr->SetOutputDisplacementFieldFileName( argv[8] );
