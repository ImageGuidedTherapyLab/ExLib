...
/*
 * This is the exact same code as for crlAffineMIRegistration with the
 * metric changed to be match cardinality.
 * A linear interpolator is used.
 *
 *    I'm think the multiresolution pyramid will low pass filter the data,
 * with a Gaussian filter, causing the label values to be screwed up.
 *
 */
...
int main( int argc, char *argv[] )
{
  ...
  typedef MatchCardinalityMetricType MetricType;
  ...
  /* Since we run this on label values, we want to use a
...
std::cerr << "Note that the multiresolution pyramid uses a Gaussian blur." 
    << "The pixel values are labels. "
    << "Match Cardinality on low pass filtered labels is not a good idea."
    << std::endl;
