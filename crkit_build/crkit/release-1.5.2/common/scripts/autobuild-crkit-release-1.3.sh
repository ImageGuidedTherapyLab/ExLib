...
# CRkit executables go here (in crkit/bin)
...
CRKIT_SOURCE=$basedir/../../crkit/trunk
unset VTK_DIR
unset QT_DIR
unset ITK_DIR
PATH=/bin:/usr/bin
...
# Place the newly built CMake first in our PATH
    PATH=$basedir/cmake/install/bin:$PATH
    export PATH
    return 0
  fi

  mkdir -p $basedir/cmake/install
  cd $basedir/cmake
...
function detect_vtk {
  ...
  then
    ...
    if [ "x${qvtk}" == "x" ] ; then
      ...
      VTK_DIR=$instdir/vtk/lib/vtk-5.2
    fi
...
function get_qt {
  ...
  ./configure -confirm-license -no-phonon -no-sql-odbc \
      -prefix $instdir/qt-${QT_VERSION} >> $logfile
  make install >> $logfile
...
# -------------------------------------------------------------
function get_crkit {
  if [ -f $instdir/bin/crlSTAPLE ] ; then
  ...
  if [ -z ${CRKIT_SOURCE} -a ! -d ${CRKIT_SOURCE} ] ; then
  ...
  $cmakebin \
    ...
    -DQT4_QGLOBAL_FILE:PATH="${QTDIR}/include/Qt/gglobal.h" \
